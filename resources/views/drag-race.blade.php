...
@push('style')
  {{-- External CSS via Vite --}}
  @vite('resources/css/drag-race.css')

  {{-- Internal Tailwind layer (components/utilities specific to this page) --}}
  <style type="text/tailwindcss">
    @layer components {
      .container-pad { @apply w-full max-w-screen-2xl 2xl:max-w-[1500px] mx-auto grid gap-4 px-[var(--gutter,12px)] py-[18px] translate-x-1.5 md:translate-x-0 lg:translate-x-[24px] xl:translate-x-[40px]; }
      .card-surface  { @apply rounded-[18px] border border-[rgb(217_226_255_/_0.25)] bg-[rgba(11,18,34,0.85)] p-3 sm:p-4 lg:p-5 backdrop-blur-[10px] shadow-[inset_1px_1px_0_rgba(255,255,255,.10),inset_-3px_-3px_0_rgba(0,0,0,.46),_0_10px_30px_rgba(0,0,0,.25)]; }
      .video-frame  { @apply relative aspect-[16/9] overflow-hidden rounded-[16px] border border-[rgb(200_214_255_/_0.45)] bg-black shadow-[0_18px_40px_rgba(0,0,0,.45),0_6px_14px_rgba(0,0,0,.25),inset_0_0_0_1px_rgba(255,255,255,.05)]; }
      .btn-ghost    { @apply h-[42px] rounded-[12px] border border-[rgb(150_170_220_/_0.6)] bg-[#0c1630]; }
      .btn-blue     { @apply h-[42px] rounded-[12px] border border-[#1f2b50] bg-[linear-gradient(180deg,#1a4a9e,#0d2b66)] text-white; }
      .btn-red      { @apply h-[42px] rounded-[12px] border border-[#3a1a1f] bg-[linear-gradient(180deg,#9a1616,#4b0a0a)] text-[#ffecec]; }
      .pill-red     { @apply inline-flex items-center justify-center rounded-full border-2 border-[#b71c1c] bg-[#3b1212] px-2 py-0.5 text-[#ffecec]; }
      .pill-blue    { @apply inline-flex items-center justify-center rounded-full border-2 border-[#0b3ca8] bg-[#11244d] px-2 py-0.5 text-[#e6f0ff]; }
      .table-wrap   { @apply overflow-auto rounded-[10px] border border-[rgb(200_214_255_/_0.45)] bg-[#0f1a33]; }
      .subtle-bar   { @apply h-2 overflow-hidden rounded-full border border-[#263553] bg-[#0c1220]; }
      .toolbar      { @apply flex gap-2 overflow-auto pb-1; }
    }
  </style>
@endpush
...
<x-layouts.app :title="__('Drag Race')">
  ...
  <div class="fixed inset-0 -z-20" style="background:
      ...
      radial-gradient(900px 320px at 90% -30%, rgba(255,255,255,.05), transparent 70%)"></div>

  <!-- PAGE -->
  <div class="container-pad">
    <!-- ===== VIDEO + SIDE ===== -->
    <section class="card-surface" style="--side:clamp(300px,32vw,420px);">
      <!-- tabs + admin toggle -->
      <div class="flex items-center justify-between gap-3 border-b border-[rgb(208_219_255_/_0.35)] pb-2">
        <div class="flex gap-1 overflow-auto px-1 sm:px-2">
          <button class="rounded-[12px] border border-[rgb(200_214_255_/_0.45)] bg-[linear-gradient(90deg,#1e3a8a_0%,_#7c0a0a_100%)] px-3 py-2 font-black text-white shadow-[0_6px_14px_rgba(64,120,255,.25)] whitespace-nowrap" data-tab="odds">ODDS</button>
          ...
          <a href="#historySection" class="rounded-[12px] border border-[rgb(200_214_255_/_0.45)] bg-[#0e1934] px-3 py-2 font-black text-white whitespace-nowrap">HISTORY</a>
        </div>
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,1fr)_var(--side)]">
        <!-- Video -->
        <div data-video class="video-frame" style="min-height:clamp(220px,36vw,520px)">
          ...
          <div id="oddsView" class="grid gap-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <div class="rounded-[22px] border border-white/10 bg-[linear-gradient(180deg,#f04e41_0%,_#a02121_100%)] p-3 text-white shadow-[0_10px_30px_rgba(0,0,0,.35),inset_1px_1px_0_rgba(255,255,255,.10),inset_-3px_-3px_0_rgba(0,0,0,.46)]">
                ...
                <div class="mt-2 grid gap-2">
                  <button data-bet="red" class="btn-ghost">BET</button>
              ...
              <div class="rounded-[22px] border border-white/10 bg-[linear-gradient(180deg,#4f89ff_0%,_#153b90_100%)] p-3 text-white shadow-[0_10px_30px_rgba(0,0,0,.35),inset_1px_1px_0_rgba(255,255,255,.10),inset_-3px_-3px_0_rgba(0,0,0,.46)]">
                ...
                <div class="mt-2 grid gap-2">
                  <button data-bet="blue" class="btn-ghost">BET</button>
            ...
            <div class="grid gap-2 rounded-[14px] border border-[rgb(150_170_220_/_0.45)] bg-[#0e1730] p-2">
              ...
              <div class="grid grid-cols-2 gap-2">
                ...
                <button id="maxAmt" class="btn-ghost">Max</button>
              </div>
            </div>
            ...
            <!-- Totals (Est. Payout line removed as requested) -->
            <div id="totalsWidget" class="grid gap-2 rounded-[14px] border border-[rgb(150_170_220_/_0.45)] bg-[#0e1426] p-2">
              ...
              <div class="grid grid-cols-2 gap-2">
                <button id="resetTotals" class="btn-ghost">Reset Totals</button>
                <button id="demoBets" class="btn-ghost">Simulate</button>
              </div>
            </div>
    ...
    <section class="card-surface">
      ...
      <div class="grid gap-3 rounded-[14px] border border-[rgb(217_226_255_/_0.25)] bg-[#0e1426] p-3">
        <div class="flex flex-wrap items-center justify-between gap-2">
          ...
          @if ($roleId === 1)
          <div class="toolbar">
            <button id="winRed"    class="btn-red px-3">Record: Red Wins</button>
            <button id="winBlue"   class="btn-blue px-3">Record: Blue Wins</button>
            <button id="undo"      class="btn-ghost px-3">Undo</button>
            <button id="startRound"class="btn-ghost px-3">Start Round</button>
            <button id="resetRound"class="btn-ghost px-3">Reset Round</button>
            <button id="clearLog"  class="btn-ghost px-3">Clear Log</button>
            <button id="exportCsv" class="btn-ghost px-3">Export CSV</button>
          </div>
          @endif
        </div>

        <div id="roadOdds" class="mx-auto -mt-1 rounded-[10px] border border-[rgb(208_219_255_/_0.35)] bg-[#0d1529] px-3 py-1.5 text-center">
          Red × 1.88 • Blue × 2.08
        </div>
        ...
        <div class="grid gap-3 md:grid-cols-2">
          <div>
            ...
            <div id="bead"
                 class="grid max-h-[calc(6*14px+12px)] grid-rows-[repeat(6,14px)] auto-cols-[14px] auto-flow-col gap-0 overflow-auto rounded-[12px] border border-[rgb(200_214_255_/_0.45)] bg-[#0f1a33] p-1.5 md:max-h-[calc(6*18px+16px)] md:grid-rows-[repeat(6,18px)] md:auto-cols-[18px]"></div>
          </div>
          <div>
    ...
    <!-- ===== Dedicated Bet History ===== -->
    <section id="historySection" class="card-surface">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <button id="hf-apply" class="btn-ghost px-3">Apply</button>
          <button id="hf-reset" class="btn-ghost px-3">Reset</button>
          <button id="hf-export" class="btn-blue  px-3">Export CSV</button>
          <button id="hf-clear"  class="btn-red   px-3">Clear</button>
        </div>
      </div>
  ...
  <div id="mbRail" class="fixed inset-x-0 bottom-0 z-[2000] hidden gap-2 border-t border-[rgb(200_214_255_/_0.45)] bg-[rgba(10,18,35,.9)] p-2 backdrop-blur-[8px] md:hidden">
    ...
    <button id="mbBet" class="btn-blue px-4 font-black">BET</button>
  </div>
    ...
    <div class="w-[min(520px,calc(100%-24px))] rounded-[16px] border border-[rgb(217_226_255_/_0.25)] bg-white text-slate-900 p-4">
      ...
      <div class="mt-3 grid gap-2">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div><div class="text-xs text-slate-600">Date</div><div id="rc-date" class="font-mono text-sm">—</div></div>
          <div><div class="text-xs text-slate-600">Side</div><div id="rc-side" class="font-bold">—</div></div>
        </div>
        <div class="grid grid-cols-3 gap-3">
          <div><div class="text-xs text-slate-600">Amount</div><div>₱ <span id="rc-amt" class="font-bold">0</span></div></div>
          <div><div class="text-xs text-slate-600">Odds (x)</div><div><span id="rc-odds" class="font-bold">0.00</span></div></div>
          <div><div class="text-xs text-slate-600">Est. Win</div><div>₱ <span id="rc-win" class="font-bold">0</span></div></div>
        </div>
        <div class="mt-2 grid place-items-center"><div id="rc-qr" class="size-[160px]"></div></div>
      </div>
      <div class="mt-4 flex justify-end gap-2"><button onclick="window.print()" class="rounded-md border px-3 py-1 text-sm">Print</button></div>
    </div>
  </div>

  <!-- Edit / Cash / Withdraw Modals -->
    ...
    <div class="w-[min(520px,calc(100%-24px))] rounded-[16px] border border-[rgb(217_226_255_/_0.25)] bg-[rgba(11,18,34,0.85)] p-4">
      ...
      <div class="grid gap-2 rounded-[12px] border border-[rgb(200_214_255_/_0.45)] bg-[#0e1730] p-3">
        <label class="grid gap-1">Display Name
          <input id="prof-name" type="text" class="h-[44px] w-full rounded-[10px] border border-[rgb(150_170_220_/_0.45)] bg-[#0f1a33] px-2"/>
        </label>
        <label class="grid gap-1">Avatar URL
          <input id="prof-avatar" type="url" class="h-[44px] w-full rounded-[10px] border border-[rgb(150_170_220_/_0.45)] bg-[#0f1a33] px-2"/>
        </label>
      </div>
      <div class="mt-3 flex justify-end gap-2">
        <button class="h-10 rounded-[12px] border border-[rgb(150_170_220_/_0.6)] bg-[#0c1630] px-3" data-close>Back</button>
        <button id="saveProfile" class="btn-blue px-3">Save</button>
      </div>
    </div>
  </div>
    ...
    <div class="w-[min(520px,calc(100%-24px))] rounded-[16px] border border-[rgb(217_226_255_/_0.25)] bg-[rgba(11,18,34,0.85)] p-4">
      ...
      <div class="grid gap-2 rounded-[12px] border border-[rgb(200_214_255_/_0.45)] bg-[#0e1730] p-3">
        <label class="grid gap-1">Amount
          <input id="cash-amt" type="number" min="1" placeholder="₱0" class="h-[44px] w-full rounded-[10px] border border-[rgb(150_170_220_/_0.45)] bg-[#0f1a33] px-2"/>
        </label>
        <label class="grid gap-1">Method
          <select id="cash-method" class="h-[44px] w-full rounded-[10px] border border-[rgb(150_170_220_/_0.45)] bg-[#0f1a33] px-2">
      ...
      <div class="mt-3 flex justify-end gap-2">
        <button class="h-10 rounded-[12px] border border-[rgb(150_170_220_/_0.6)] bg-[#0c1630] px-3" data-close>Back</button>
        <button id="cashSubmit" class="btn-red px-3">Confirm</button>
      </div>
    </div>
  </div>
  ...
  <div id="modal-withdraw" class="modal fixed inset-0 z-[1200] hidden place-items-center bg-[rgba(0,0,0,.45)]" role="dialog" aria-hidden="true">
    <div class="w-[min(520px,calc(100%-24px))] rounded-[16px] border border-[rgb(217_226_255_/_0.25)] bg-[rgba(11,18,34,0.85)]">
      ...
      <div class="mx-4 mb-4 grid gap-2 rounded-[12px] border border-[rgb(200_214_255_/_0.45)] bg-[#0e1730] p-3">
        <label class="grid gap-1">Amount
          <input id="wd-amt" type="number" min="1" placeholder="₱0" class="h-[44px] w-full rounded-[10px] border border-[rgb(150_170_220_/_0.45)] bg-[#0f1a33] px-2"/>
        </label>
        <label class="grid gap-1">Destination
          <select id="wd-method" class="h-[44px] w-full rounded-[10px] border border-[rgb(150_170_220_/_0.45)] bg-[#0f1a33] px-2">
            <option>GCash</option><option>PayMaya</option><option>Bank Transfer</option>
          </select>
        </label>
      </div>
      <div class="mx-4 mb-4 flex justify-end gap-2">
        <button class="h-10 rounded-[12px] border border-[rgb(150_170_220_/_0.6)] bg-[#0c1630] px-3" data-close>Back</button>
        <button id="wdSubmit" class="btn-blue px-3">Request</button>
      </div>
    </div>
  </div>
  ...
  <script>
    // role
    const ROLE_ID = {{ json_encode($roleId) }};
    // noop for removed sound/desktop notifs
    function addNotif() {}
  </script>

  <!-- YouTube init + admin show/hide -->
  <script>
    (function(){
      const YT_ID='mzBv3fUDxRA';
      const EMBED=`https://www.youtube.com/embed/${YT_ID}?rel=0&modestbranding=1&playsinline=1`;
      const iframe=document.getElementById('yt');
      if(iframe){ iframe.src=EMBED; iframe.style.display='block'; iframe.addEventListener('load', syncSideHeight); }
    })();
    ...
    (function(){
      ...
      const iframe=document.getElementById('yt');
      if(!iframe) return;
      const apply=(on)=>{ iframe.style.display=on?'block':'none'; overlay&&overlay.classList.toggle('hidden', on); };
      if(!chk){ apply(true); return; }
      apply(chk.checked);
  ...
  <script>
    ...
    function sync(){
      ...
      el('pot2').textContent=fmt(pot);
      el('tR2').textContent=fmt(state.red);
      el('tB2').textContent=fmt(state.blue);
    ...
    function addToHistory({side, amount, odds}){
      ...
      saveAll(); renderHistorySidebar(); renderHistoryTable();
    }
    function renderHistorySidebar(){
      const tbody=el('betHistory'); if(!tbody) return;
      tbody.innerHTML='';
      if(history.length===0){
      ...
      history.slice(0,20).forEach(h=>{
        const pill=h.side==='Red'?'pill-red':'pill-blue';
        const tr=document.createElement('tr');
        tr.innerHTML=`<td>${h.timeStr}</td>
                      <td><span class="${pill}">${h.side}</span></td>
                      <td>₱${fmt(h.amount)}</td>
    ...
    function exportHistoryCSV(list){
      ...
      const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='bet-history.csv'; a.click(); URL.revokeObjectURL(a.href);
    ...
    /* dedicated history filters/pager */
    const HF_PAGE={ size:12, now:1 };
    function getHFFilters(){ return { side:(el('hf-side')?.value||'all'), min:Math.max(0,Number(el('hf-min')?.value||0)) }; }
    ...
    (function(){
      ...
      el('mbBet').addEventListener('click', ()=>{
        ...
        state[side]+=v; el('mbAmount').value='';
        recalcOdds(); sync();
    ...
    /* milestones (no sound) */
    let lastMilestone=0;
    function checkMilestones(pot){ const step=1000; const m=Math.floor(pot/step); if(m>lastMilestone){ lastMilestone=m; } }
    ...
    function renderLog(){
      ...
      logs.slice(start, start + PAGE_SIZE).forEach(row=>{
        const pill=row.winner==='Red'?'pill-red':'pill-blue';
        const tr=document.createElement('tr');
        tr.innerHTML = `
          <td>${row.time}</td>
    ...
    function renderBead(){
      ...
      for(let c=0;c<COLS;c++) for(let r=0;r<ROWS;r++){ const d=document.createElement('div'); d.style.gridRow=(r+1); cell(d); bead.appendChild(d); }
      seq.forEach((v,i)=>{ const col=Math.floor(i/ROWS), row=i%ROWS; const idx=col*ROWS+row; const d=bead.children[idx]; if(d){ cell(d, v==='R'?'red':'blue'); d.textContent=String(i+1); } });
    }
    function renderBig(){
      const big=el('big'); if(!big) return; big.innerHTML='';
      for(let c=0;c<COLS;c++) for(let r=0;r<ROWS;r++){ const d=document.createElement('div'); d.style.gridRow=(r+1); cell(d,null,true); big.appendChild(d); }
    ...
    function undo(){ if(!seq.length) return; seq.pop(); logs.shift(); renderBead(); renderBig(); renderLog(); saveAll(); }
    ...
    function startRound(seconds=60){
      ...
      tHandle=setInterval(()=>{ tLeft--; setTimerText(formatSecs(Math.max(tLeft,0))); if(tLeft<=0){ clearInterval(tHandle); tHandle=null; setTimerText('00:00'); } },1000);
    }
    function resetRound(){ clearInterval(tHandle); tHandle=null; tLeft=0; setTimerText('—'); }
    ...
    document.getElementById('clearLog')?.addEventListener('click', ()=>{
      logs = []; el('logBody') && (el('logBody').innerHTML = ''); curPage = 1; renderLog(); toast('Logs cleared');
    });
