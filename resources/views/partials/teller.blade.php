...
<x-layouts.app :title="__('Billiard')">
  ...
  <style>
    ...
    #history-wrap{
      background:#0f172a;              /* solid slate-900 style */
    ...
    #history-list{
      max-height: 560px;               /* scrollable content area */
      overflow:auto;
    ...
    .hist-pill{
      ...
      height:24px; min-width:24px; padding:0 8px; border-radius:999px;
      border:1px solid rgba(255,255,255,.16); font-size:12px; font-weight:600;
      background:#111827;
    }
    ...
    /* =========================================================
       SOLIDIFY: Video+Logrohan & Bet Sections (no removals)
       (Bet cards: original colors; Chips: original colors)
    ==========================================================*/

    /* Solid wrapper backgrounds for sections 1 & 2 */
    ...
    /* Inside Video+Logrohan */
    .logro-zone{
      background:#0b1223 !important;
      border:1px solid rgba(255,255,255,.12) !important;
    }

    /* Bet section inner panels (pct bar, amount+chips, mini road) */
    main > .grid > section:nth-child(2) .rounded-xl{
      background:#0b1223 !important;
    ...
    main > .grid > section:nth-child(2) .h-3{
      border:1px solid rgba(255,255,255,.16) !important;
    }

    /* Mobile logro stack: solid din for consistency */
    .md\:hidden .logro-zone{
      background:#0b1223 !important;
      border:1px solid rgba(255,255,255,.12) !important;
    }
  ...
  <!-- ========================================================
       MAIN: [VIDEO+LOGROHAN | BET SECTION | BET HISTORY (paginated)]
  ========================================================= -->
  <main class="max-w-screen-2xl 2xl:max-w-[2400px] mx-auto p-4">
    <div class="grid gap-6 md:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)_360px]">
      ...
      <!-- CENTER: BET SECTION (Bet % bar, Bet cards, Bet Amount + Chips, Mini Road) -->
      ...
      <!-- RIGHT: BET HISTORY (SOLID background, high contrast) -->
      <aside id="history-wrap" class="p-0">
        ...
        <!-- Empty -->
        <div id="history-empty" class="px-4 py-6 text-center text-slate-300 hidden">
          No bets yet.
        </div>
        ...
        <div class="px-4 py-3 border-t border-white/10 bg-[#0d1426] flex items-center justify-between">
          ...
          <div class="text-sm text-slate-200">
            Page <span id="history-page">1</span> / <span id="history-pages">1</span>
          </div>
  ...
  <div id="print-ticket" class="hidden">
    <div id="print-ticket-content" class="mx-auto max-w-[440px] bg-white text-black rounded-lg p-5 shadow">
      <!-- filled by JS -->
    </div>
  </div>
  ...
  <script>
    ...
    (function(){
      ...
      function encode(text){
        ...
        return Array.from(text).map(patternFor).join('n'); // inter-char narrow space
      }
    ...
    /* ====== ORIGINAL STATE & HELPERS (unchanged functions) ====== */
    const players=[
    ...
    function streakRuns(seq){
      const out=[]; for(const t of seq){
        if(!out.length || out[out.length-1].t!==t) out.push({t,n:1}); else out[out.length-1].n++;
      } return out;
    }
    ...
    function addToHistory(entry){
      ...
      renderHeaderHistory();
      const dot=document.getElementById('header-history-dot') || document.getElementById('history-dot');
    ...
    function printTicket(ticket){
      const holder=document.getElementById('print-ticket-content');
      const wrap=document.getElementById('print-ticket');
...
New Balance: ${currentBalance.toLocaleString()}.`);
    ...
    window.onload=()=>{
      ...
      try{
        const saved=localStorage.getItem('bk_video_hidden');
        if(saved==='1'){ videoWrap.classList.add('hidden'); if(toggleLbl) toggleLbl.textContent='Show Video'; }
      }catch(e){}
