...
<x-layouts.app :title="__('Drag Race — Teller Mode')">
  ...
  <body class="m-0 min-h-dvh overflow-y-auto overflow-x-hidden bg-[#050a15] font-[Inter,system-ui,-apple-system,Segoe_UI,Roboto,Helvetica,Arial] text-[13px] text-slate-100">
    ...
    {{-- ===== Centered desktop canvas that scales with Tailwind (no JS scaling) ===== --}}
        ...
        <main class="h-[780px] w-[1320px] p-4 grid gap-4 grid-cols-[300px_minmax(0,1fr)_320px]">
          ...
          <section class="space-y-4 rounded-[16px] border border-white/10 bg-[#0c1326e6] p-4 backdrop-blur-md shadow-[0_10px_30px_rgba(0,0,0,.25),inset_1px_1px_0_rgba(255,255,255,.06)]">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="rounded-[18px] border border-white/10 bg-[linear-gradient(180deg,#f04e41_0%,_#a02121_100%)] p-4 text-white shadow-[0_10px_30px_rgba(0,0,0,.35),inset_1px_1px_0_rgba(255,255,255,.10),inset_-3px_-3px_0_rgba(0,0,0,.46)]">
              ...
              <div class="rounded-[18px] border border-white/10 bg-[linear-gradient(180deg,#4f89ff_0%,_#153b90_100%)] p-4 text-white shadow-[0_10px_30px_rgba(0,0,0,.35),inset_1px_1px_0_rgba(255,255,255,.10),inset_-3px_-3px_0_rgba(0,0,0,.46)]">
                <div class="flex min-w-0 items-center justify-between gap-2">
            ...
            {{-- ROADS ONLY (steady; manual scroll) --}}
    ...
    {{-- ===== JS (logic only; no layout scaling) ===== --}}
    <script>
      // Utils
      ...
      function recordBet(side, amount){
        if(amount<10){ toast('Minimum bet is ₱10'); return; }
      ...
      document.addEventListener('keydown',e=>{ if(e.key==='r') recordBet('red',100); if(e.key==='b') recordBet('blue',100); });
      el('scoreRed').addEventListener('click', ()=>recordBet('red',100));
      el('scoreBlue').addEventListener('click',()=>recordBet('blue',100));
      ...
      // Init
      (function init(){ initGrid('beadGrid'); initGrid('bigGrid'); renderBead(); renderBig(); recalc(); })();
    </script>
